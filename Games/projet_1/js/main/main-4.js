// MAIN JS
var game = new Phaser.Game('100%', 800, Phaser.CANVAS, '', { preload: preload, create: create, update: update, render: render });
...
function create () { 
    ...
    // bg2.anchor.setTo(game.world.centerX, 0);
    // bg3.anchor.setTo(game.world.centerX*2, 0);
  ...
  function while_tic(pause, switchGlobal, ticGlobal) {
	...
	for (var i = 0; i < level.length; i++) {
	    for (var j = 0; j < level[i].length; j++) {
	        ...
	        if (level[i][j] == 'x') {
	            var wall = game.add.sprite(28*j, 28*i, 'ground');
	            platforms.add(wall);
    ...
    player.animations.add('left', [0, 2 ,1 ,3], 8, true);
  	player.animations.add('jumpL', [8, 7, 6, 5], 12, true);
  	player.animations.add('jumpR', [14, 15, 16, 17], 12, true);
...
function inputs (idledetect) {
    ...
    else if (cursors.right.isDown)
    {
      	...
      	mountainsBack.tilePosition.x -= 0.04;
    	mountainsMid1.tilePosition.x -= 0.2;
    	mountainsMid2.tilePosition.x -= 0.6;  
        //  Move to the right
        player.body.velocity.x = 200;
 		idledetect=1;
      if (player.body.touching.down)
